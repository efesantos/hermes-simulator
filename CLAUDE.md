...
- Run the benchmark: `.venv/bin/python -m simulator` (drives the default candidate
  field against the `dana` persona). `scripts/build_report.py <run_id>` rebuilds a
  ranked report from on-disk results if a run was interrupted.
- `simulator/harness.py` is the only module that shells out to `hermes`; the grader
  reads world state out-of-band (never through an agent tool).
