...
def test_http_server_serves_seeded_world(tmp_path: Path):
    ...
    with serve("simulator.world.calendar_server", world_db, port):
        assert wait_listening(port), "calendar server never bound its port"
        tool_names, result = asyncio.run(call_tool(url, "list_events", {}))
...
def test_http_server_on_occupied_port_fails_fast(tmp_path: Path):
    ...
    port = free_port()
    with serve("simulator.world.calendar_server", world_db, port):
        assert wait_listening(port), "first server never bound its port"
        with serve("simulator.world.calendar_server", world_db, port) as second:
